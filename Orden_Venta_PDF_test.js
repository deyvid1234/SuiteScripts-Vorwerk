function Orden_Venta_PDF(request,response) 
{
	...
	var titleForm	= 'Orden de Venta';
		...
		try
		{
			...
			var name			= nlapiEscapeXML(returnBlank(customer.getFieldValue('companyname')));
			...
			serie		= returnBlank(nlapiEscapeXML(salesorder.getLineItemValue('item', 'serialnumbers', 1)));
			if(serie == '')
			{
				serie	= 'N/A';
			}
			...
			var Pie	= '';
				...
				Pie	+='<table>';
					Pie += '<tr width="100%">';
					 ...
					 Pie += "<table  align='left'>";
					 	Pie += "<tr><td width='300px' class='AvisoFac'><u><b>IMPORTANTE</b></u> Si requiere CFDi (Factura Electrónica) "+
						" cuenta con 10 días para solicitar su CFDi (Factura Electrónica) siempre que se encuentre dentro del mes de compra. "+
						"Si la compra la realizó  dentro de los últimos 5 días del mes, deberá solicitar su CFDi (Factura Electrónica) a más tardar "+
						"el último día hábil del mes del compra. No se realizarán Facturas en meses posteriores a la compra.</td></tr>";
					 Pie += '</table>';
					Pie += '</td>';
			    	Pie += '</tr>';
				Pie	+='</table>';
				...
				var xml = "<?xml version='1.0' encoding='UTF-8'?>\n<!DOCTYPE pdf PUBLIC '-//big.faceless.org//report' 'report-1.1.dtd'>\n";
					xml += "<pdf>";
						xml += "<head>";
							xml += "<style>";
								xml += ".AvisoFac{border: 1px solid red; font-size:10px;} ";
