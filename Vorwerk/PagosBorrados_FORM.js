function PagosBorrados_FORM(request, response)
{
	var titleForm			= 'Pagos Borrados';
	try
	{
		...
		if (request.getMethod() == "GET")
		{
			...
			var help_mensaje						= 'Este es el contenido del correo electrónico de <i>' + titleForm + '</i>.que será enviado cada que se registre un pedido sin múltiples pagos';
				...
				help_mensaje	 			 	   += '<br>[tranid] ==  Pago';
				help_mensaje	 			 	   += '<br>[trandate] ==  Fecha';
				help_mensaje	 			 	   += '<br>[url] ==  Enlace del registro de la Orden de Venta';
				help_mensaje	 			 	   += '<br>[salesorder] ==  Orden de Venta';
			...
			{
				var urlSUITELET				= nlapiResolveURL("SUITELET", "customscript_pagos_borrados_form", "customdeploy_pagos_borrados_form", false);
				var	url 					= urlSUITELET;
			...
			{
				form.setScript('customscript_pagos_borrados_form_fv');
			}
		...
		else
		{
			...
			switch(mode)
			{
				case 'edit':
				{
			    	...
			    	var dataFile					= nlapiCreateFile('PagosBorrados_FORM_CONFIG.xml', 'XMLDOC', xmlString);
			...
			nlapiSetRedirectURL('SUITELET','customscript_pagos_borrados_form', 'customdeploy_pagos_borrados_form', false, params_edit);
	...
	catch(e)
    {
		...
		nlapiSetRedirectURL('SUITELET','customscript_pagos_borrados_he', 'customdeploy_pagos_borrados_he', false, params_handler_error);
