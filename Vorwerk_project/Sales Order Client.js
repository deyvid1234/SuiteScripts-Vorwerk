...
define(['N/record','N/search','N/https','N/runtime','N/currentRecord','N/ui/dialog'],
    ...
    function(record,search,https,runtime,currentRecord,dialog) {
        ...
        /**
         * Función helper para detectar el modo de ejecución
         * @param {Object} scriptContext - El contexto del script
         * @returns {string} - 'create', 'edit', 'copy' o 'unknown'
         */
        function getExecutionMode(scriptContext) {
            // Método 1: Usar scriptContext.mode directamente
            if (scriptContext.mode) {
                return scriptContext.mode;
            }
            
            // Método 2: Verificar si el record tiene ID (si no tiene, es create)
            try {
                ...
                console.log('Error obteniendo record ID:', e);
            }
            
            // Método 3: Verificar el contexto de ejecución
            var context = runtime.executionContext;
            if (context === 'USERINTERFACE') {
                // En la interfaz de usuario, necesitamos más validación
                try {
                    var currentRecordObj = currentRecord.get();
                    if (currentRecordObj && currentRecordObj.id) {
                        return 'edit';
                    } else {
                        return 'create';
                    }
                } catch (e) {
                    console.log('Error obteniendo current record:', e);
                }
            }
