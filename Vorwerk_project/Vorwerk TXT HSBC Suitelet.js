...
function(record, search, email, render, file,Dictionary) {
    ...
    function GenerarArchivoTXT(context){
        try{
            ...
            var f = new Date();
            var ano_2 = f.getFullYear().toString().slice(-2)
            ...
            var d = f.getDate();
            var fecha_6 = d+m+ano_2
            log.debug('fecha_6',fecha_6)
            if(m<10){
                m= "0"+m;
            }
            if(d<10){
            ...
            var fechaS = meses[f.getMonth()]+' '+ f.getFullYear().toString().substr(-2)
            var importe         = 0;
            ...
            busqueda.run().each(function(r){
                ...
                objSearchResult[r.id] = {
                    ...
                    custentity_ban_prov: r.getValue('custentity_ban_prov'),
                    custentity_ce_rfc: r.getValue('custentity_ce_rfc'),
                };
                ...
                objCompensation[r.id].altname = r.getValue('altname'),
                objCompensation[r.id].custentity_ban_prov = r.getValue('custentity_ban_prov'),
                objCompensation[r.id].custentity_ce_rfc = r.getValue('custentity_ce_rfc')
            ...
            for(idEmp in objCompensation){
                ...
                var altname        		= objCompensation[idEmp]['altname'];
                var custentity_ban_prov	= objCompensation[idEmp]['custentity_ban_prov'];
                var custentity_ce_rfc	= objCompensation[idEmp]['custentity_ce_rfc'];
                log.debug('objCompensation[idEmp]',objCompensation[idEmp]);
                log.debug('altname ',altname)
                /*if(altname.length > 34){
                	altname = altname.substr(0, 34)
                }else if(altname.length < 34){
                	for(i= altname.length ;i < 34; i++){
                		altname = altname + ' ';
                	}
                }*/
                log.debug('altname post',altname)
                if(monedero == 'F' || !monedero){
                    ...
                    var ref1OB      = getRef35(registro);
                    var ref2OB      = getRef35(registro);
                    var ref3OB      = getRef35(registro);
                    var ref4OB      = getRef35(registro);
...
//                        metodoPAgo  = '002';
//                    }
                    var cuentaVW = '4037669694'
                    var beneficiario    = getNameBen(firstname,lastname);
                    if(custentity_ban_prov == 4 ){
                    	numCuentaAux
                    	log.debug('numCuentaAux',numCuentaAux)
                    	var numeroCuenta = numCuentaAux.substr(7, 10)
                    	log.debug('numeroCuenta',numeroCuenta)
                    	//numeroCuenta = getAccount_20(numeroCuenta)
                    	//log.debug('numeroCuenta 20',numeroCuenta)
                    	lineasAplicadas++;
                        r3 += lineasAplicadas+','+cuentaVW +','+numeroCuenta+','+getVal(objCompensation[idEmp]['total'])+','+ "1" +','+ 'COMPENSACION ' + fechaS+',' + altname.toUpperCase() + ',0'; 
                        r3 += String.fromCharCode(13) + String.fromCharCode(10); 
                    }else{
                    	lineasAplicadasOB++
                        
                        numeroCuenta = getAccount_20(numCuentaAux)
                        log.debug('numeroCuenta 20',numeroCuenta)
                    	r3OB += cuentaVW+','+'VME060622GL2,'+numeroCuenta+','+numCuentaAux.substr(0,3)+','+ altname.toUpperCase()+',' + getVal(objCompensation[idEmp]['total'])+','+fecha_6+','+ 'COMPENSACION ' + fechaS; 
                        r3OB += String.fromCharCode(13) + String.fromCharCode(10); 
                    }
...
//                var r2              = "2"   + "1" + "001" + importe + "01" +numcuenta + getNumAbo_6(lineasAplicadas);
                var r4              = "1" + "," + lineasAplicadas +','+'COMPENSACION '+fechaS;
                var fileContents    = r3 + r4 + String.fromCharCode(13) + String.fromCharCode(10);
                var fileName        = fecha+consecutivo+".csv";
            ...
            if(lineasAplicadasOB!=0){
//                var r1              = "1"   + "000082282802" + fecha + consecutivo + "VORWERK MEXICO S DE RL DE CV        " +  "TEST                " + "15" + "D" + "01";
//                var r2              = "2"   + "1" + "001" + importe + "01" +numcuenta + getNumAbo_6(lineasAplicadas);
//                var r4              = "1" + "," + lineasAplicadas +','+fechaS;
                var fileContents    = r3OB;
                var fileName        = fecha+consecutivo+"OB.csv";
                //var file            = nlapiCreateFile(fileName, "PLAINTEXT", fileContents);
                var fileTXT = file.create({
                    name: fileName,
                    fileType: file.Type.PLAINTEXT,
                ...
                log.debug('fileTXTOB',fileTXT);
                //var fileValue = file.getValue();
                log.debug('archivo generado' , 'archivo generado' );
                /*response.setContentType('PLAINTEXT', fileName, 'attachment');
                response.write(fileValue);*/
                var renderer = render.create();
                intFileIdOB = fileTXT.save();
                
                log.debug('fileTXTOB',intFileIdOB);
                //var fileValue = file.getValue();
                log.debug('archivo generado' , 'archivo generado' );
            ...
            var idfile
            if(intFileId && intFileIdOB){
            	idfile = intFileId +'-'+ intFileIdOB
            }else if (intFileId){
            	idfile = intFileId
            }else if(intFileIdOB){
            	idfile = intFileIdOB
            }
            context.response.write(JSON.stringify({id:idfile}));
        }
        catch(e){
    ...
    function getRFC_18( v ){
        var ceros18 ='000000000000000000';
        var f = ceros18.length - v.length;
        var n = ceros18.slice(0,(f));
            n = n.concat(v);
        return n;
    }
    function getRef35(v){
    ...
    function getNum_18( v ){
        var ceros6 ='000000000000000000';
        var f = ceros6.length - getVal(v.toString().length);
        var n = ceros6.slice(0,f);
            n = n.concat(v);
        return n;
